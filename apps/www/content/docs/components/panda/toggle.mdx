---
title: Toggle
description: A two-state button that can be either on or off.
links:
  doc: https://base-ui.com/react/components/toggle
  api: https://base-ui.com/react/components/toggle#api-reference
---

import ToggleDemo from "@/examples/toggle-demo";
import ToggleOutline from "@/examples/toggle-outline";
import ToggleWithText from "@/examples/toggle-with-text";
import ToggleDisabled from "@/examples/toggle-disabled";

<ComponentPreview base="panda" name="toggle-demo">
  <ToggleDemo />
</ComponentPreview>

## Installation

<Tabs defaultValue="cli">

<TabsList>
  <TabsTrigger value="cli">CLI</TabsTrigger>
  <TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">

```bash
npx shadcn@latest add https://shadcn-cssinjs.com/r/toggle.json
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

```bash
pnpm add @base-ui/react @stylexjs/stylex
```

<Step>
  Make sure [StyleX is configured](/docs/installation) and the design tokens are
  installed.
</Step>

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource name="toggle" title="toggle.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</Tabs>

## Usage

```tsx
import { Toggle } from "@/components/ui/toggle/toggle";
```

```tsx
<Toggle aria-label="Toggle bold">
  <BoldIcon />
</Toggle>
```

## Examples

### Outline

Use the `outline` variant for a bordered toggle.

<ComponentPreview base="panda" name="toggle-outline">
  <ToggleOutline />
</ComponentPreview>

### With Text

<ComponentPreview base="panda" name="toggle-with-text">
  <ToggleWithText />
</ComponentPreview>

import ToggleSizes from "@/examples/toggle-sizes";

### Size

<ComponentPreview base="panda" name="toggle-sizes">
  <ToggleSizes />
</ComponentPreview>

### Disabled

<ComponentPreview base="panda" name="toggle-disabled">
  <ToggleDisabled />
</ComponentPreview>

import ToggleRtl from "@/examples/toggle-rtl";

## RTL

To enable right-to-left support, see the [RTL guide](/docs/rtl).

<ComponentPreview base="panda" name="toggle-rtl" direction="rtl">
  <ToggleRtl />
</ComponentPreview>

## API Reference

See the [Base UI](https://base-ui.com/react/components/toggle#api-reference)
documentation for the full API.
