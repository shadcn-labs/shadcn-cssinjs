...
// explicitly, returns every registry configured in the project, including the
// built-in StyleX registry. Shared by the CLI command and the MCP server so
...
export function resolveSearchRegistries(
  ...
  return Object.keys(config?.registries ?? BUILTIN_REGISTRIES)
}
