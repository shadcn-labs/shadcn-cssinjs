  ...
  try {
    ...
    if (registry.include?.length) {
      throw new RegistryValidationError(
        `Registry catalog "${name}" uses "include", but consumer registry endpoints must serve a resolved registry catalog. Run "npx shadcn-cssinjs build" and serve the built registry.json, or use loadRegistry() in a dynamic route.`,
        {
          context: {
            registry: name,
...
export async function getShadcnRegistryIndex() {
  try {
    ...
    return registryIndexSchema.parse(registrySchema.parse(result).items)
  } catch (error) {
    logger.error("\n")
    handleError(error)
  }
